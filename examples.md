...
From `step-security/cirruslabs-cache@v3` onwards, you can now exclude unwanted packages with [exclude pattern](https://github.com/actions/toolkit/tree/main/packages/glob#exclude-patterns)
...
```
`Get npm cache directory` step can then be used with `step-security/cirruslabs-cache` as shown below
...
```yaml
- name: Set up Python
  # The step-security/cirruslabs-cache step below uses this id to get the exact python version
  id: setup-python
...
Caching gems with Bundler correctly is not trivial and just using `step-security/cirruslabs-cache`
